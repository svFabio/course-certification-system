...
class PreinscriptionComponent extends Component
{
    ...
    public bool $preinscriptionOpen = false;

    public function mount(int $group, CourseService $courseService): void
    {
        $this->groupId = $group;
        $this->group = Group::with('course')->findOrFail($group);
        $courseService->closeExpiredPreinscriptions();
        $this->group->course->refresh();
        $this->preinscriptionOpen = $courseService->isPreinscriptionOpen($this->group->course);
    }
    ...
    public function render()
    {
        return view('livewire.preinscription-component', [
            'group' => $this->group,
            'preinscriptionOpen' => $this->preinscriptionOpen,
        ]);
    }
}
