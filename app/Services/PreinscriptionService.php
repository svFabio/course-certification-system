...
class PreinscriptionService
{
    ...
    {
        $group = Group::with('course')->findOrFail($data['group_id']);
        $course = $group->course;

        $this->courseService->closeExpiredPreinscriptions();
        $course->refresh();

        if (! $this->courseService->isPreinscriptionOpen($course)) {
            throw ValidationException::withMessages([
                'group_id' => 'La preinscripción de este curso está cerrada o aún no inicia.',
            ]);
        }
