            ...
            @foreach ($courses as $course)
                <div class="card-umss flex flex-col justify-between overflow-hidden">
                    <div class="p-6">
                        ...
                        <div class="space-y-1.5 text-xs text-[#4A4A4A] pt-3 border-t border-[#E5E5E5]">
                            ...
                            @if ($course->fecha_inicio_preinscripcion && $course->fecha_fin_preinscripcion)
                                <p><span class="font-medium text-[#121212]">Preinscripción:</span>
                                    {{ $course->fecha_inicio_preinscripcion->format('d/m/Y') }}
                                    —
                    ...
                    <div class="p-4 bg-[#F5F5F5] border-t border-[#E5E5E5]">
                        @if (! $course->isPreinscriptionOpen())
                            <span class="btn-primary w-full text-center opacity-50 cursor-not-allowed">
                                Preinscripción no disponible
                            </span>
                        @elseif ($course->groups->isNotEmpty())
